...
class classe_util:
	...
	def decnet(self, out, anchors, mask):
		"""
		PARAMS:
	...
	def filtrer_boxes(self, boxes, box_confidences, box_class_probs, class_threshold=.6):
		"""
	...
	def nm_suppression(self, boxes, scores, iou_threshold=.5):
	...
	def treatnetout(self, yhat, shape, masks, anchors):
	...
	# CHARGER UNE IMAGE.
	def load_image_pixels(self, filename, shape=(416, 416)):
	...
	def draw_plot_boxes(self, filename, boxes, labels_indices, scores, labels):
		# CHARGER L'IMAGE;
	...
	def predict_and_plot(self, filename):
		img = cv2.imread(filename)
		image, _, _ = self.load_image_pixels(filename)
		yhat = self.model.predict(image, steps=1)
		pred_boxes, pred_classes, pred_scores = self.treatnetout(yhat, img.shape, self.masks, self.anchors)
		self.draw_plot_boxes(filename, pred_boxes, pred_classes, pred_scores, self.labels)




	# DÉFINIR UNE FONCTION QUI DÉSSINE LES RÉSULTATS ET ÉCRIT UN FICHIER IMAGE.
	def draw_output_boxes(self, filename, boxes, labels_indices, scores, labels, directory = "output"):
		...
		for i in range(len(boxes)):
			...
			org = (start_point[0], start_point[1])
			fontScale = .55
			text = labels[labels_indices[i]] + ' [' + str(scores[i])[:5] +']'
	...
	# DÉFINIR UNE FONCTION QUI PRÉDIT ET QUI PLOT LES RÉSULTATS.
	def predict_and_write_image(self, filename, directory = "output"):
		image, _, _ = self.load_image_pixels(filename)
		img = cv2.imread(filename)
		yhat = self.model.predict(image, steps=1)
		pred_boxes, pred_classes, pred_scores = self.treatnetout(yhat, img.shape, self.masks, self.anchors)
		self.draw_output_boxes(filename, pred_boxes, pred_classes, pred_scores, self.labels)


	# DÉFINIR UNE FONCTION QUI PRÉDIT ET QUI PLOT LES RÉSULTATS.
	def predict_and_return(self, f, shape):
		yhat = self.model.predict(f, steps = 1)
		return self.treatnetout(yhat, shape, self.masks, self.anchors)
